...
public class ViewStudent extends AppCompatActivity implements OnClickListener {
    ...
    private void initViews() {
        ...
        DatabaseHelper dbHelper = new DatabaseHelper(ViewStudent.this);
        studentBeanList = new ArrayList<>(dbHelper.getAllStudent());
        studentAdapter = new StudentAdapter(this, studentBeanList, this);
    ...
    ItemTouchHelper.SimpleCallback callback = new ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT | ItemTouchHelper.RIGHT) {
        ...
        public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
            DatabaseHelper db = new DatabaseHelper(ViewStudent.this);
            final int position = viewHolder.getAdapterPosition();
            db.deleteStudent(studentAdapter.getmStudentList().get(position));
            studentAdapter.removeItem(position);
            Snackbar.make(findViewById(R.id.card_recyclerView), "Student Deleted!!!", Snackbar.LENGTH_LONG).show();
        }
    };
}