...
public class UploadAttendance extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dbHelper = new DatabaseHelper(UploadAttendance.this);
        ...
        btnSubmit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (selectedFilePath != null && isExcelFile(selectedFilePath)) {
                    List<String> res = dbHelper.uploadExcelFileToDatabase(UploadAttendance.this, selectedFilePath, subjectId, -1, date, day, month, yearInt);
                    if(res.size() == 0) {
                        Toast.makeText(UploadAttendance.this, "Some error has occured!!!", Toast.LENGTH_LONG).show();
                    } else {
                        Toast.makeText(UploadAttendance.this, "Successful: " + res.get(0) + "\nUnsuccessful: " + res.get(1), Toast.LENGTH_LONG).show();
                        setInitialValues();
                        buildDatePicker();
                        btnDate.requestFocus();
                    }
                } else {
                    Toast.makeText(UploadAttendance.this, "Invalid Excel file", Toast.LENGTH_LONG).show();
                }
            }
        });
    }
